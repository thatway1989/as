...
int main(int argc,char **argv)
{
  ...
  unsigned char data[2][16] = {
	...
	p_doip = (Doip_Message *)sendline;
	if(id == 0){ //acvive route
		send_len = 7;
		DoIP_Util_FillGenHdr(DOIP_HDR_TYPE_ROUT_ACTIV_REQ, send_len, p_doip);
	} else { //diag req
		send_len = 6;
		DoIP_Util_FillGenHdr(DOIP_HDR_TYPE_DIAG_REQ, send_len, p_doip);
	}
	memcpy(p_doip->data, data[id], send_len);
